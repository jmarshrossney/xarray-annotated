...
returning a domain-shaped payload; this is the one cross-domain reader that
collapses both into a single uniform value.  Where the writer `annotate` turns
facet values into an `Annotated` hint, `declarations_from_signature` turns a whole
signature back into a facet value per declared DataArray — it is the read-side
counterpart to `annotate`, and their round-trip is exact.

The unified value `Declared` is deliberately *homogeneous*: every slot holds a
marker (`Unit` / `Dims` / `Dtype` / `Coords`) or `None`.  Markers are the
package's public currency — the schema markers are load-bearing (they carry
strictness that has no bare-string form) — so a facet-generic consumer wants one
marker per slot rather than a mix of strings and objects.  A bare-string unit
shorthand is normalised to a `Unit`, so the shape does not depend on how the unit
was spelled.

Like `annotate`, this is cross-domain — it assembles the units `Unit` marker and
the schema `Dims` / `Dtype` / `Coords` markers into one value — so it lives at the
package root rather than in either domain.
"""
...
class Declared:
    ...
    The field order (`unit`, `dims`, `dtype`, `coords`) matches `annotate`'s
    keyword arguments, so `annotate(unit=d.unit, dims=d.dims, dtype=d.dtype,
    coords=d.coords)` rebuilds the hint this was read from.
    """
...
def _annotated_declaration(hint: Any) -> Declared | None:
    ...
    Delegates to the existing `annotated_unit` and `annotated_schema` so the
    "is this an `Annotated[DataArray, ...]` hint?" logic is not duplicated, then
    folds their payloads into a single homogeneous value.  Returns `None` when the
    hint declares no facet, which is `walk_signature`'s "no declaration" signal.
    """
    ...
    if unit is None and dims is None and dtype is None and coords is None:
        return None
    return Declared(
